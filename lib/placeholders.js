...
const urlPlaceholders = function (obj) {
  ...
  const result = {
    ...
    title: slugify(obj.data.slug, 'pretty', true) || slugify(obj.basename_without_ext, 'pretty', true),
    slug: slugify(obj.data.slug) || slugify(obj.basename_without_ext),
    categories: obj.data.categories.map(category => category.toLowerCase()).join('/'),
